import {OptionType, parseOption, runNodeCli} from "./node-cli";
...
describe("Run CLI", () => {
	...
	it("should expose a function", () => {
		expect(runNodeCli).toBeDefined();
	});

	it("should not throw errors", () => {
		expect(() => runNodeCli(load)).not.toThrow();
		expect(() => runNodeCli(config)).not.toThrow();
	});
	...
	it("should return the expected output", () => {
		...
		const output = runNodeCli(config) as DotenvConfigOutput;
